...
function sessionPath(id) { return path.join(SESSIONS_DIR, id); }
...
async function connectInstance(instanceId) {
  ...
  const socket = makeWASocket({ version, auth: { creds: state.creds, keys: makeCacheableSignalKeyStore(state.keys, baileysLogger) }, printQRInTerminal: false, logger: baileysLogger, browser: ['Chrome (Linux)', 'Chrome', '121.0.0'], generateHighQualityLinkPreview: false, syncFullHistory: false });
  ...
  socket.ev.on('connection.update', async (update) => {
    ...
    if (qr) {
      ...
      } catch (err) { logger.error(`[${instanceId}] Erro QR: ${err.message}`); }
    }
    if (connection === 'open') {
    ...
    if (connection === 'close') {
      ...
      if (shouldReconnect && entry && entry.retries < 5) {
        ...
        await sleep(delay);
        connectInstance(instanceId).catch((e) => logger.error(`[${instanceId}] Reconexão falhou: ${e.message}`));
      } else { activeSockets.delete(instanceId); }
    }
  });
...
async function disconnectInstance(instanceId) {
  const entry = activeSockets.get(instanceId);
  if (entry) { try { await entry.socket.logout(); } catch (_) {} activeSockets.delete(instanceId); }
  const sessDir = sessionPath(instanceId);
  if (fs.existsSync(sessDir)) fs.rmSync(sessDir, { recursive: true, force: true });
}

function getSocket(instanceId) { return activeSockets.get(instanceId)?.socket || null; }
...
```
Commit ✅ depois vai para o último.

---

## PASSO 11 — src/whatsapp/messageQueue.js

**Nome do arquivo:**
```
src/whatsapp/messageQueue.js
